...
async function onApproveCallback(data, actions) {
  try {
    ...
    if (errorDetail?.issue === "INSTRUMENT_DECLINED" && !data.card && actions) {
      ...
      let errorMessage;
      if (transaction) {
        errorMessage = `Transaction ${transaction.status}: ${transaction.id}<br><br>${JSON.stringify(orderData, null, 2)}`;
      } else if (errorDetail) {
      ...
      resultMessage(
        `Transaction ${transaction.status}: ${transaction.id}<br><br>See console for all available details<br><br>${JSON.stringify(orderData, null, 2)}`,
      );
      console.log(
        "Capture result",
        orderData,
